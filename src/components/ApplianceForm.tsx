...
const COMMON_APPLIANCES = [
  { name: 'Refrigerator', category: 'Kitchen', wattage: 150, hoursPerDay: 24, daysPerMonth: 30 },
  { name: 'Air Conditioner', category: 'Heating & Cooling', wattage: 3500, hoursPerDay: 8, daysPerMonth: 30 },
  { name: 'LED Light Bulb', category: 'Lighting', wattage: 10, hoursPerDay: 6, daysPerMonth: 30 },
  { name: 'Television (LED)', category: 'Electronics', wattage: 100, hoursPerDay: 5, daysPerMonth: 30 },
  { name: 'Laptop', category: 'Electronics', wattage: 65, hoursPerDay: 8, daysPerMonth: 30 },
  { name: 'Washing Machine', category: 'Laundry', wattage: 1000, hoursPerDay: 1, daysPerMonth: 10 },
  { name: 'Water Heater', category: 'Water Heating', wattage: 4000, hoursPerDay: 3, daysPerMonth: 30 },
  { name: 'Microwave', category: 'Kitchen', wattage: 1200, hoursPerDay: 0.5, daysPerMonth: 25 },
];
...
export default function ApplianceForm({ appliances, onAddAppliance, onRemoveAppliance, onUpdateAppliance }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/70 backdrop-blur-sm rounded-xl p-6 border border-white/20">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Quick Add Common Appliances</h2>
          ...
          {COMMON_APPLIANCES.map((appliance, index) => (
            ...
            >
              <div className="flex items-center space-x-2 mb-1">
                ...
                <span className="font-medium text-gray-900 text-sm">{appliance.name}</span>
              </div>
              <p className="text-xs text-gray-600">{appliance.wattage}W • {appliance.hoursPerDay}h/day</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white/70 backdrop-blur-sm rounded-xl p-6 border border-white/20">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">Your Appliances</h2>
          <button
        ...
        {appliances.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm">Add some appliances to see your power bill prediction!</p>
          </div>
        ) : (
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="font-medium text-gray-900">{appliance.name}</h3>
                      <p className="text-sm text-gray-600">
                        {appliance.category} • {appliance.wattage}W • {appliance.hoursPerDay}h/day • {appliance.daysPerMonth} days/month
                      </p>
                    </div>
                  </div>
                </div>
